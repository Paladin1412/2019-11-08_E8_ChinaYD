...
jQuery(function(){
	...
	jQuery.ajax({
		type:"get",
		async: false,
		url: "/mobile/plugin/weaversj/workflow/js/personalDeviceCheck/getDemoDataCheck.jsp?zccode="+zccode,
...
function getChecked(){
	...
	for(var i =0;i<fields.length;i++){
		...
		var demoVal = fieldidAndVal[fieldName];
		if(hasSpecial(tableVal) != hasSpecial(demoVal)){
			flagse = false;
			break;
		}
	}

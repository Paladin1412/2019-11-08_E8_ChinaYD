		...
		if(jQuery(this).val()==="0"){
			if(!getChecked()){
				top.Dialog.confirm(userAlert, function(){
	    			...
	    			//Query("#"+fieldChage_id).val("");
	    			//checkinput2('field173769','field173769span',jQuery(this).attr("viewtype"));
	    			//jQuery(this).change();
	    		}, 320, 90,true);
			}else{
				setTimeout(function(){
					var getDetailVal = jQuery("#"+detail).val();
					jQuery("#"+detail+"_readonlytext").html(getDetailVal.replace(/\r\n/g, "<br/>").replace(/\n/g, "<br/>"));
			...
			var getDetailVal = jQuery("#"+detail).val();
			jQuery("#"+detail).val(getDetailVal.replace(/<br\/>/g, "\r\n") );
		}
	});
			...
			if(jQuery("#"+fieldChage_id).val()==="0"){
		    	...
		    	if(!getChecked()){
					top.Dialog.confirm(userAlert, function(){
						setChecked();
						var getDetailVal = jQuery("#"+detail).val();
						jQuery("#"+detail).val(getDetailVal.replace(/\r\n/g, "<br/>").replace(/\n/g, "<br/>"));
...
function getChecked(){
	...
	for(var i =0;i<fields.length;i++){
		...
		if(tableVal != demoVal){
			...
			if(fieldTypeToGet == 2){
				if(tableVal.replace(/\s/g, "") != demoVal.replace(/\s/g, "")){
					flagse = false;
					break;
				}
			}else if(fieldTypeToGet == 4 ){

				if(tableVal.replace(/\s/g, "").replace(/<br\/>/g, "").replace(/\n/g, "") != demoVal.replace(/\s/g, "").replace(/<br\/>/g, "").replace(/\n/g, "")){
					flagse = false;
					break;
				}
			}else{
...
function setFieldVal(fieldId,fieldType,fieldval){
	if(fieldType === '4'){
		jQuery("#"+fieldId).val(fieldval.replace(/\r\n/g, "<br/>"));
		jQuery("#"+fieldId+"_readonlytext").html(fieldval.replace(/\r\n/g, "<br/>").replace(/\n/g, "<br/>"));

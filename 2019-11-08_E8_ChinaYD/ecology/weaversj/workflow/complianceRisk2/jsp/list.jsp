...
WeaverSJUtil wUtil = new WeaverSJUtil();
	...
	if(!baseUtil.getCheckRoleResource("合规风险通知书权限", user_new.getUID())){
	...
	String Procurement_Entity = user.getLanguage()==8?"Notified Entity":"被通知部门";	
	
	String bg=user.getLanguage()==8?"Collaborative Office":"战略综合";
	String zc=user.getLanguage()==8?"Daily Administration":"合规管理";	
	String titlenameS=user.getLanguage()==8?"Compliance penalty letter of intent process":"合规风险通知书";
	...
	if(!Department_or_department_s.equals("")){
		sqlWhere+=" and a.Notified_Department like '%"+Department_or_department_s+"%' ";
	}
	...
	<BODY>		
		...
		<wea:layout type="diycol">
			...
			<%
				...
				String backfields =" a.*,h1.lastname as CreateName,h2.SUBCOMPANYNAME as  CompanyName  "; 				
				...
				String tableString =" <table pagesize=\""+perpage+"\"  tabletype=\"checkbox\">"+
						...
						"	<head>"+
								...
								//标题					
								"<col width=\"120px\"  text=\""+Procurement_Entity+"\"   column=\"Notified_Department\" transmethod=\"weaversj.workflow.activity.WorkflowMethodUtil.getDepartMentName\"  orderkey=\"Notified_Department\"/>"+
								//部门/地区
